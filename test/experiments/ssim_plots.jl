...
to noise ratios.
"""
function threshold_ssim(img,L,P,Q,noise,step_size,type)
    ...
    for s in step
        ...
        ac_ssim = assess(SSIM(), img, thresh_img)
        append!(ssim_list, ac_ssim)
    end

    return step, ssim_list
end
...
"""
function threshold_ssim_plot(img,low_noise,high_noise,step)
    x_label = "Threshold"
    y_label = "SSIM"

    a, b = threshold_ssim(img, L, P, Q, low_noise, step, "soft")
    c, d = threshold_ssim(img, L, P, Q, low_noise, step, "hard")
    e, f = threshold_ssim(img, L, P, Q, high_noise, step, "soft")
    g, h = threshold_ssim(img, L, P, Q, high_noise, step, "hard")
...
`step`: step at which to increase the threshold
"""
function threshold_ssim_plot_leg(img,low_noise,high_noise,step)
    x_label = "Threshold"
    y_label = "SSIM"
...
Q = qfilter(H)
P = pfilter(H)
plot_list=[threshold_ssim_plot(img_list[i],0.1,0.7,0.1) for i=1:length(img_list)]
...
airplane_leg = threshold_ssim_plot_leg(img_list[1], 0.1, 0.7, 0.1)
