...
function L4Σ(X::Array{Float64,2}, wt::OrthoFilter, n::Integer, noise::Float64;
    ...
    y₁ = maketrees(size(Y₀,1),size(Y₀,3),4)
    # Find threshold
    if TH == SoftTH()
...
function L4testdenoise(x::Vector{Float64}, noise::Float64;
    ...
    @inbounds begin
        for n in ProgressBar(1:N)
            X₁, Y₀, Y₁, th = L4Σ(X₀, wt, n, noise, TH=TH, method=method)
...
function L4testdenoise_noshift(x::Vector{Float64}, noise::Float64;
                     TH::Wavelets.Threshold.THType=HardTH(), N::Integer=50, M::Integer=100,
                     method::Symbol=:shozen)
    L₁ = Array{Float64,2}(undef,(N,3))
    L₂ = Array{Float64,2}(undef,(N,3))
